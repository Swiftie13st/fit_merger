...
fn main() -> Result<(), String> {
    ...
    // 保存合并后的文件
    fs::write("../fit_files/all_rides_merged.fit", merged_data)
        .map_err(|e| format!("写入文件失败: {}", e))?;

    println!("合并完成！文件已保存为: ../fit_files/all_rides_merged.fit");
...
fn verify_merged_file() -> Result<(), String> {
    ...
    let data = fs::read("../fit_files/all_rides_merged.fit")
    ...
    println!("验证结果:");
    println!("  文件大小: {} 字节", file.header.data_size + 14 + 2); // 数据大小 + 头部 + CRC
