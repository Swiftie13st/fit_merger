...
fn main() -> Result<(), String> {
                ...
                match parser.parse() {
                    Ok(fit_file) => {
                        ...
                        for message in &fit_file.messages {
                            match message {
                                fit_merger::fit_types::FitMessage::Data(data_msg) => {
                                    // 只保留非空的消息
                                    if !data_msg.fields.is_empty() {
                                        filtered_messages.push(message.clone());
                                    }
                                }
                                fit_merger::fit_types::FitMessage::Definition(_) => {
                                    // 保留所有定义消息
                                    filtered_messages.push(message.clone());
                                }
                            }
                        ...
                        // 创建新的FIT文件，只包含有效消息
                        let mut filtered_file = fit_file;
                        filtered_file.messages = filtered_messages;
                        
                        println!("  有效消息数量: {}", filtered_file.messages.len());
                        
                        if !filtered_file.messages.is_empty() {
                            merger.add_file(filtered_file);
                            successful_files += 1;
                        } else {
                            println!("  跳过空文件");
                        }
    ...
    // 保存合并后的文件
    fs::write("../fit_files/all_rides_final.fit", merged_data)
        .map_err(|e| format!("写入文件失败: {}", e))?;

    println!("合并完成！文件已保存为: ../fit_files/all_rides_final.fit");
...
fn verify_merged_file() -> Result<(), String> {
    ...
    let data = fs::read("../fit_files/all_rides_final.fit")
